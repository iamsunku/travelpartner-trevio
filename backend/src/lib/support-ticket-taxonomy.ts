...
const OPS_SET = new Set(OPERATIONS_TYPES.map((o) => o.value));
const DELIVERY_SET = new Set(DELIVERY_TYPES.map((d) => d.value));

export function isValidOperationsType(value: unknown): value is OperationsTypeValue {
  return typeof value === "string" && OPS_SET.has(value as OperationsTypeValue);
}

export function isValidDeliveryType(value: unknown): value is DeliveryTypeValue {
  return typeof value === "string" && DELIVERY_SET.has(value as DeliveryTypeValue);
}
