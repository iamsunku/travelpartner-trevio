...
import { WALLET_TXNS } from "@/lib/mock-data";
import {
...
export function WalletView() {
  ...
  const balance = 845000;
  const commissionThisMonth = WALLET_TXNS.filter((t) => t.type === "Credit" && t.source === "Commission").reduce((s, t) => s + t.amount, 0);
  const totalCredited = WALLET_TXNS.filter((t) => t.type === "Credit").reduce((s, t) => s + t.amount, 0);
  const totalDebited = WALLET_TXNS.filter((t) => t.type === "Debit").reduce((s, t) => s + t.amount, 0);

  const filtered = WALLET_TXNS.filter((t) => {
    ...
    <div className="space-y-5">
      ...
      <Card>
        ...
        <CardContent className="p-0">
          ...
          <p className="text-[11px] text-muted-foreground mt-2 px-4 pb-4">Showing {filtered.length} of {WALLET_TXNS.length} transactions</p>
        </CardContent>
      </Card>
    </div>
  );
