...
export function AgenciesView() {
  ...
  function handleAdd() {
    ...
    toast({ title: "Agency created", description: `${newAgency.name} added on Trial plan.` });
  }

  function toggleStatus(id: string) {
    setAgencies((prev) => prev.map((a) => a.id === id ? { ...a, status: a.status === "Suspended" ? "Active" : "Suspended" } : a));
    const ag = agencies.find((a) => a.id === id);
    toast({ title: ag?.status === "Suspended" ? "Agency activated" : "Agency suspended", description: `${ag?.name} status updated.` });
  }
