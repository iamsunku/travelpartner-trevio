...
import {
  Plane, Building2, Bus, Train, Palmtree, Stamp, ShieldCheck,
...
const SERVICE_ICON: Record<string, React.ElementType> = {
  Flight: Plane, Hotel: Building2, Bus: Bus, Train: Train,
  Holiday: Palmtree, Visa: Stamp, Insurance: ShieldCheck,
};
...
function BookingDetailDialog({ booking, open, onOpenChange }: { booking: Booking | null; open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  function action(label: string, desc: string, variant?: "default" | "destructive") {
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto scroll-thin">
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            ...
            <Button variant="outline" size="sm" className="text-amber-600" onClick={() => action("Refund initiated", `Refund request created for ${booking.bookingRef}`)}>
              <IndianRupee className="w-3.5 h-3.5 mr-1" /> Refund
            </Button>
            <Button variant="outline" size="sm" className="text-rose-600 ml-auto" onClick={() => action("Booking cancelled", `${booking.bookingRef} cancelled`, "destructive")}>
...
export function BookingsView() {
  ...
  }, [activeTab, serviceFilter, search, dateFrom, dateTo]);
          ...
          <div className="flex flex-col lg:flex-row gap-2 mb-3">
            ...
            <Select value={serviceFilter} onValueChange={setServiceFilter}>
              ...
              <SelectContent>
                ...
                {["Flight", "Hotel", "Bus", "Train", "Holiday", "Visa", "Insurance"].map((s) => (
