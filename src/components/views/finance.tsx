...
function OverviewTab() {
  ...
  const netProfit = totalRevenue - totalExpenses - tdsDeducted;
...
function GstTab() {
  const totalTaxable = GST_FILINGS.reduce((s, g) => s + g.taxable, 0);
  const outputTax = Math.round(totalTaxable * 0.18);
...
function InvoicesTab() {
  ...
  function openInv(inv: typeof INVOICES[number]) { setSelected(inv); setOpen(true); }
